...
async function getRecipeDetails(recipe_id) {
    ...
    const createdRecipe = await DButils.execQuery(
        `SELECT *
         FROM Personal_Recipes
         WHERE id = ${recipe_id}`
      );
    console.log("creatdRecipe?");
    console.log(createdRecipe);
    if (createdRecipe.length > 0) {
        return {
            id: createdRecipe[0].id,
            title: createdRecipe[0].title,
            readyInMinutes: createdRecipe[0].readyInMinutes,
            image: createdRecipe[0].image,
            popularity: createdRecipe[0].popularity,
            vegan: createdRecipe[0].vegan,
            vegetarian: createdRecipe[0].vegetarian,
            glutenFree: createdRecipe[0].glutenFree,
            instruction: createdRecipe[0].instruction,
            ingredients: createdRecipe[0].ingredients,
            servings: createdRecipe[0].servings        
        }
    }
